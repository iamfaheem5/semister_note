"""
Document Parsers for Cover Letter Skill.

Vendored copy aligned with latex-paper-en/scripts/parsers.py for the shared
core (DocumentParser ABC, LatexParser, TypstParser, free helpers). The
``get_parser`` factory is the canonical .tex / .typ dispatch; PDF support is
intentionally absent because cover-letter only ingests LaTeX manuscripts and
